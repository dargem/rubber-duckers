...
class TweeterClient:
    ...
    async def _auto_like_with_all_accounts(self, post_id: int, posting_account_username: str):
        ...
        if tasks:
            like_results = await asyncio.gather(*[task for _, _, task in tasks], return_exceptions=True)
            
            # Log results for each account
            successful_likes = 0
            for (i, username, _), result in zip(tasks, like_results):
                if isinstance(result, Exception):
                    logger.error(f"Account #{i+1} ({username}) FAILED to like post {post_id}: {result}")
                else:
                    logger.info(f"Account #{i+1} ({username}) SUCCESSFULLY liked post {post_id}")
                    successful_likes += 1
            
            logger.info(f"Auto-like complete: {successful_likes}/{len(tasks)} eligible accounts liked post {post_id}")
            logger.info(f"Expected likes on website: {successful_likes} (including self-like)")
            return successful_likes
        else:
            logger.warning("No eligible accounts for auto-liking (all accounts filtered out)")
            return 0
    ...
    async def send_reply(self, reply:str, post_id):
        ...
        logger.info(f"Attempting to reply ({len(reply)} chars): {reply[:100]}{'...' if len(reply) > 100 else ''}")
        try:
